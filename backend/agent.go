...
func NewAgent(client *LiteLLMClient) *Agent {
	return &Agent{
		client: client,
...
func (a *Agent) PostMessage(ctx context.Context, chatID, userMessage string) (*AgentReply, error) {
	...
	// Snapshot history under the lock, then release it for the (slow) LLM
	// call — a chat is only ever driven by one user, so this is safe.

...
Rules:
- Add an invariant ONLY when the user explicitly, firmly settled a constraint or decision in THIS exchange AND it would actually constrain a future proposal or estimate — a stack choice, an architecture decision, a business rule about what gets built, a scope/budget limit stated as final. "We'll only use Go on the backend", "no third-party auth providers", "this task must not exceed 40 hours" all qualify.
- Do NOT add team/process facts that don't constrain what gets proposed or estimated, even when stated just as firmly — release schedule, which design tool the team uses, meeting cadence, who owns what role. Those may belong in project notes (a separate, softer memory this call does not manage) but are never a reason to refuse or reshape a technical proposal, so they are never invariants.

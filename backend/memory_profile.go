...
// profileSystemPrompt asks for the single global user profile — name, style,
// format, constraints — folded in from one exchange. Unlike known_stack/
// notes (facts about a project), every field here describes the USER, not
// the work, so the bar for writing anything is "the user explicitly said
// this about themselves or how they want to be treated," never an inference
// from the exchange's topic or tone.
...
Given the current profile (a JSON object, possibly with empty fields) and the latest exchange (one user message and the assistant's reply), return the UPDATED complete profile as a single JSON object: {"name": "...", "style": "...", "format": "...", "constraints": ["..."]}.
...
- All text values (name, style, format, constraints) are in Russian, regardless of what language the exchange itself was in — except "name" itself, which is written exactly as the user gave it.
...
func (a *Agent) updateProfileAfterTurn(ctx context.Context, chatID, userMessage, assistantReply string) *UserProfile {
	...
	log.Printf("agent: profile updated (name=%q, %d constraint(s))", updated.Name, len(updated.Constraints))
